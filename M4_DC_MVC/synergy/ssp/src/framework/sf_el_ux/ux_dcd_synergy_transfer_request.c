...
UINT  ux_dcd_synergy_transfer_request(UX_DCD_SYNERGY *dcd_synergy, UX_SLAVE_TRANSFER *transfer_request)
{
    ...
    if (transfer_request -> ux_slave_transfer_request_phase == (ULONG)UX_TRANSFER_PHASE_DATA_OUT)
    {
        ...
        if ((dcd_synergy->ux_dcd_synergy_transfer.ux_synergy_transfer_tx == NULL)
                || (ed -> ux_dcd_synergy_ed_fifo_index == UX_SYNERGY_DCD_FIFO_C) /* If the pipe is control pipe */
                || ((transfer_request -> ux_slave_transfer_request_requested_length) <= /* or if data-pipe and size is less */
                        (endpoint -> ux_slave_endpoint_descriptor.wMaxPacketSize)))  /* than or equal to end-point size */
        {
            ...
            /* If the endpoint is a Control endpoint, all this is happening under Interrupt and
             * there is nothread to suspend.  */
            if (ed -> ux_dcd_synergy_ed_index != 0UL)
            {
        ...
        else
        {
            ...
            /* Clear the BRDY and BEMP status for this pipe.  */
            ux_dcd_synergy_register_clear(dcd_synergy, UX_SYNERGY_DCD_BEMPSTS,
                    (USHORT)(1UL << ed -> ux_dcd_synergy_ed_index));
            ux_dcd_synergy_register_clear(dcd_synergy, UX_SYNERGY_DCD_BRDYSTS,
                    (USHORT)(1UL << ed -> ux_dcd_synergy_ed_index));
    ...
    else
    {
        ...
        if ((dcd_synergy->ux_dcd_synergy_transfer.ux_synergy_transfer_rx == NULL) /* if DMA is not used */
                || (ed -> ux_dcd_synergy_ed_fifo_index == UX_SYNERGY_DCD_FIFO_C) /* If the pipe is control pipe */
                || (ed -> ux_dcd_synergy_ed_index == 0UL) /* Control pipe (one more check?) */
                || ((transfer_request -> ux_slave_transfer_request_requested_length) <= /* or if data-pipe and size is less */
                        (endpoint -> ux_slave_endpoint_descriptor.wMaxPacketSize)))  /* than or equal to end-point size */
