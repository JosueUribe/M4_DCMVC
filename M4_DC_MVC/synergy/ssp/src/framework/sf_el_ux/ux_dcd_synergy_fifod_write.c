...
UINT  ux_dcd_synergy_fifod_write_dma (UX_DCD_SYNERGY *dcd_synergy, UX_DCD_SYNERGY_ED *ed, UINT dma_bytes_to_transfer)
{
    ...
    ULONG    fifo_sel;
    UX_DCD_SYNERGY_PAYLOAD_TRANSFER payload;
    volatile ULONG pipectr_reg;
    USHORT pipe_number;
    ULONG endpoint_buf_size;
    volatile ULONG buf_ready_wait_count;
    UINT sem_get_status;
    ...
    sem_get_status = _ux_utility_semaphore_get(&dcd_synergy->dma_tx_completion_semaphore, UX_WAIT_FOREVER);

    /* Clear DREQE bit */
    ux_dcd_synergy_register_clear (dcd_synergy, UX_SYNERGY_DCD_D0FIFOSEL, (USHORT)UX_SYNERGY_DCD_DFIFOSEL_DREQE);

    if (sem_get_status != (UINT)UX_SUCCESS)
    {
        return (UX_SYNERGY_DCD_FIFO_WRITE_ERROR);
    }

    /* Update buffer address and length for rest of bytes. */
    payload.payload_buffer += payload.transfer_times * payload.transfer_width * payload.transfer_blocks;
    payload.payload_length -= (ULONG) (payload.transfer_times * payload.transfer_width * payload.transfer_blocks);
    ...
    do
    {
        pipectr_reg = ux_dcd_synergy_register_read(dcd_synergy,UX_SYNERGY_DCD_PIPE1CTR + ((pipe_number - 1UL) * 2UL));
                                                    ...
                                                    VOID * p_fifo_sel)
{
    ...
    ux_dcd_synergy_register_set(dcd_synergy, UX_SYNERGY_DCD_PIPE1CTR + ((pipe_used_for_dma - 1UL) * 2UL), (USHORT)1);
}
/*******************************************************************************************************************//**
