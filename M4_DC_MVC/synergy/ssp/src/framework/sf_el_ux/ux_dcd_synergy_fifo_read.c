...
UINT  ux_dcd_synergy_fifo_read_dma (UX_DCD_SYNERGY *dcd_synergy, UX_DCD_SYNERGY_ED *ed, UINT dma_bytes_to_transfer)
{
    ...
    if (0UL != payload.payload_length)
    {
        ...
        /* Ready with the calculation before the DMA completes... */
        payload.payload_buffer += (payload.transfer_times * payload.transfer_width *
        		payload.transfer_blocks);
        payload.payload_length -= (ULONG) (payload.transfer_times * payload.transfer_width *
        		payload.transfer_blocks);
